...
from app.services.asset_service import (
    create_asset_service, delete_asset_service, get_asset_service, list_all_assets_service, update_asset_service
)

router = APIRouter()

@router.post("/", response_model=AssetRead, tags=["assets"])
def create_asset(asset: AssetCreate, session: Session = Depends(get_session)):
    new_asset = create_asset_service(asset, session)
    return new_asset

@router.get("/{asset_id}", response_model=AssetRead, tags=["assets"])
def get_asset(asset_id: int, session: Session = Depends(get_session)):
    asset = get_asset_service(asset_id, session)
    return asset

@router.put("/{asset_id}", response_model=AssetRead, tags=["assets"])
...
@router.delete("/{asset_id}", response_model=dict, tags=["assets"])
def delete_asset(asset_id: int, session: Session = Depends(get_session)):
    delete_asset_service(asset_id, session)
    return {"message": "Asset deleted successfully"}

@router.get("/", response_model=list[AssetRead], tags=["assets"])
def list_all_assets(session: Session = Depends(get_session)):
    assets = list_all_assets_service(session)
    return assets

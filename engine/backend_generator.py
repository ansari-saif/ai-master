...
def get_ai_response(json_data):
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            "here is one sample\n"
            "backend/app/api/v1/routes/todos.py\n\n"
            "```\n"
            ...
            "    create_todo_service, delete_todo_service, get_todo_service, list_all_todos_service, update_todo_service\n"
            ")\n\n"
            "router = APIRouter()\n\n"
            "@router.post(\"/\", response_model=TodoRead, tags=[\"todos\"])\n"
            "def create_todo(todo: TodoCreate, session: Session = Depends(get_session)):\n"
            "    new_todo = create_todo_service(todo, session)\n"
            "    return new_todo\n\n"
            "@router.get(\"/{todo_id}\", response_model=TodoRead, tags=[\"todos\"])\n"
            "def get_todo(todo_id: int, session: Session = Depends(get_session)):\n"
            "    todo = get_todo_service(todo_id, session)\n"
            "    return todo\n\n"
            "# Update Todo by ID\n"
            "@router.put(\"/{todo_id}\", response_model=TodoRead, tags=[\"todos\"])\n"
            ...
            "    return updated_todo\n\n"
            "# Delete Todo by ID\n"
            "@router.delete(\"/{todo_id}\", response_model=dict, tags=[\"todos\"])\n"
            "def delete_todo(todo_id: int, session: Session = Depends(get_session)):\n"
            "    delete_todo_service(todo_id, session)\n"
            "    return {\"message\": \"Todo deleted successfully\"}\n\n"
            "# List All Todos\n"
            "@router.get(\"/\", response_model=list[TodoRead], tags=[\"todos\"])\n"
